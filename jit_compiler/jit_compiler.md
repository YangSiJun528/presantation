...
저번주에 갑작스럽게 발표를 맞게 되서
발표 주제를 많이 고민했었는데,
여러 분야의 사람들이게 유익하고,
가볍게 볼 수 있었으면 해서
많은 프로그래밍 언어에서 사용되는 
Just In Time Compiler라는 개념을
주제로 결정했습니다.
...
<!--
인터프리터 언어의 대표적인 문제점은 느리다는 건데요.

인터프리터는 
코드를 한 줄씩 읽고 해석하는 과정을 거쳐야 합니다.
따라서 미리 기계어로 컴파일 된 실행파일을 사용하는
컴파일러 방식에 비해 느립니다.
...
그리고 
자바나 C#처럼 미리 컴파일된 중간 언어와 가상머신을 사용하는 언어도 
컴파일 과정을 거치지만 실행 단계에서 인터프리터와 같은 특성을 가지고 있습니다.
따라서 인터프리터와 동일하게 속도가 낮다는 문제를 가지고 있고, 
문제를 해결하기 위해 Just In Time 컴파일러를 사용합니다.
-->
- - -
...
실행 전에 모든 코드를 미리 컴파일하지 않고, 실제 실행 시 필요한 부분만을 컴파일하여 실행하는 컴파일러를 말합니다. 
컴파일 대상이 아닌 코드는 인터프리터가 담당하고요.

프로그램 실행 중인 런타임 시점에 코드를 컴파일하기 때문에 
Just In Time 컴파일러 라고 부릅니다.

컴파일 시에 최적화를 진행하고요.
컴파일 한 코드를 캐싱하여 재사용합니다.
이러한 점 때문에 인터프리터만 사용하는 것 보다 높은 성능을 낼 수 있습니다.

용어가 혼동되지 않기 위해서 
런타임 시점 이전에 전체 코드를 컴파일하는
기존의 컴파일러는 Ahead Of Time 컴파일러라고 부릅니다.
...
컴파일을 해야 할 정도로 최적화가 필요하지 않고, 
이 경우 오히려 성능이 낮아질 수 있습니다.
-->
...
반복문의 반복 횟수를 증가시켜가면서 실행하였습니다.

그리고 결과를 말하기 전에 말씀드리고 싶은게 있는데
python3와 pypy는 여러 차이가 존재하기 때문에
결과의 원인이 온전히 JUST IN TIME 컴파일러에 의한 건 아닐 수도 있습니다.
-->
- - -
...
반복 횟수가 1억을 넘기 시작하면서 
시간 차이가 크게 벌어지는 모습을 확인할 수 있는데요.

이 차이는 반복 횟수가 증가할수록 벌어지는 모습을 보여줍니다.
